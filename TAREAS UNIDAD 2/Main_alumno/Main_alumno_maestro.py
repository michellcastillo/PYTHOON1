...
class Tareas:
    ...
    def __init__(self, ventana_tarea):
        ...
        self.window.config(bd=10, bg="DarkSeaGreen1")


        titulo = Label(ventana_tarea, text="REGISTRATE", fg="black", bg="DarkSeaGreen1", font=("Arial", 20, "bold"), pady=10).pack()

        marco = LabelFrame(ventana_tarea, text="DATOS DEL MAESTRO", bg="DarkSeaGreen1", font=("Arial", 15, "bold"), pady=5)
        marco.config(bd=2, bg="DarkSeaGreen1")
        marco.pack()


        label_asignatura= Label(marco, text="NOMBRE: ", bg="DarkSeaGreen1", font=("Ariual", 12, "bold")).grid(row=0, column=0,sticky='s', padx=5,pady=9)
        self.combo_asignatura = Entry(marco, width=25, font=("Arial", 12, "bold"))
        self.combo_asignatura.grid(row=0, column=1, padx=5, pady=0)

        label_fecha = Label(marco, text="APELLIDO: ", bg="DarkSeaGreen1", font=("Arial", 12, "bold")).grid(row=1,column=0,sticky='s',padx=5,pady=8)
        self.fecha = Entry(marco, width=25, font=("Arial", 12, "bold"))
        self.fecha.grid(row=1, column=1, padx=5, pady=8)

        label_puntaje = Label(marco, text="MATERIA: ", bg="DarkSeaGreen1", font=("Arial", 12, "bold")).grid(row=2, column=0,sticky='s', padx=5,pady=8)
        self.puntaje = Entry(marco, width=25, font=("Arial", 12, "bold"))
        self.puntaje.grid(row=2, column=1, padx=5, pady=8)

        label_descripcion = Label(marco, text="HORAS: ", bg="DarkSeaGreen1", font=("Arial", 12, "bold")).grid(row=3, column=0,sticky='s',padx=5, pady=8)
        ...
        frame_botones.config(bg="DarkSeaGreen1")
        frame_botones.pack()

        boton_registrar = Button(frame_botones, text="REGISTRAR",command=self.agregar_tareas, height=2, width=10,bg="SkyBlue1", fg="black", font=("Arial", 12, "bold")).grid(row=0, column=1, padx=10,pady=15)
        boton_eliminar = Button(frame_botones, text="ELIMINAR", command=self.eliminar_asignatura,height=2, width=10,bg="SkyBlue1", fg="black", font=("Arial", 12, "bold")).grid(row=0, column=2, padx=10,pady=15)
        boton_eliminar = Button(frame_botones, text="REGRESAR", command=self.llamar_login, height=2, width=10,bg="SkyBlue1", fg="black", font=("Arial", 12, "bold")).grid(row=0, column=4, padx=10,pady=15)
        ...
        self.tree.heading("columna2", text='MATERIA', anchor=CENTER)
        self.tree.column("columna2", width=150, minwidth=75, stretch=NO)

        self.tree.heading("columna3", text='HORAS', anchor=CENTER)
